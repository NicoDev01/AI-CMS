...
import { Page, EditorMode, Media } from '../../types';
...
export default function PageEditor({ page, onSave, onCancel, mode, media }: PageEditorProps) {
  const [title, setTitle] = useState('');
  const [content, setContent] = useState('');
  const [status, setStatus] = useState<'published' | 'draft'>('draft');
  const [slug, setSlug] = useState('');
  const [template, setTemplate] = useState<'standard' | 'with-sidebar' | 'blog'>('standard');
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-webmen-text-primary mb-8">{mode === 'edit' ? 'Seite bearbeiten' : 'Neue Seite erstellen'}</h1>
      ...
      <div className="space-y-6">
        ...
        <div>
          <label htmlFor="template" className="block text-sm font-medium text-webmen-text-secondary">Template</label>
          <select
            id="template"
            value={template}
            onChange={(e) => setTemplate(e.target.value as 'standard' | 'with-sidebar' | 'blog')}
            className="mt-1 block w-full bg-webmen-background border border-webmen-border rounded-md shadow-sm py-2 px-3 text-webmen-text-primary focus:outline-none focus:ring-webmen-accent focus:border-webmen-accent sm:text-sm"
          >
            <option value="standard">Standard-Layout</option>
            <option value="with-sidebar">Layout mit Seitenleiste</option>
      ...
      <div className="mt-8 flex justify-end space-x-4">
        ...
        <button onClick={() => handleSave(true)} className="bg-webmen-primary text-white font-semibold py-2 px-4 rounded-lg hover:bg-webmen-accent transition-colors">
            {mode === 'edit' && page?.status === 'published' ? 'Aktualisieren' : 'Veröffentlichen'}
        </button>
      </div>
